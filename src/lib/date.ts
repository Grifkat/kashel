import { т, текущійЯзык } from '../i18n'

/*
 * Названия месяцев и дней — на языке окна. По-английски даты идут в
 * британском виде: «14 September 2026». Это обычный английский и ложится
 * в те же шаблоны, что русское «14 сентября 2026», без переделки каждого.
 */
const EN_ДАТЫ = текущійЯзык() === 'en'

export const MONTHS = EN_ДАТЫ
  ? ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
  : ['Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь', 'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь']
export const MONTHS_GEN = EN_ДАТЫ
  ? MONTHS
  : ['января', 'февраля', 'марта', 'апреля', 'мая', 'июня', 'июля', 'августа', 'сентября', 'октября', 'ноября', 'декабря']
export const MONTHS_SHORT = EN_ДАТЫ
  ? ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
  : ['янв', 'фев', 'мар', 'апр', 'май', 'июн', 'июл', 'авг', 'сен', 'окт', 'ноя', 'дек']
export const WEEKDAYS = EN_ДАТЫ ? ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'] : ['Вс', 'Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб']
/**
 * Название посреди фразы: «удалить расходы за сентябрь 2026».
 * По-русски — строчными целиком. По-английски месяцы и дни недели пишутся с
 * большой буквы, поэтому там опускается только первая буква обычного слова
 * («This month» → «this month»), а «September 2026» остаётся как есть.
 */
export function вСтрочную(s: string): string {
  if (!EN_ДАТЫ) return s.toLowerCase()
  const первое = s.split(/[\s,]/)[0]
  if (MONTHS.includes(первое) || WEEKDAYS_FULL.includes(первое)) return s
  return s.charAt(0).toLowerCase() + s.slice(1)
}

export const WEEKDAYS_FULL = EN_ДАТЫ
...
export function relDate(s: string): string {
  ...
  if (d === 0) return т('сегодня')
  if (d === 1) return т('вчера')
  if (d === 2) return т('позавчера')
  if (d === -1) return т('завтра')
  return humanDate(s)
}
