...
const ЗНАКИ: Record<string, Подлинникъ> = {
  anna: П(petr, т('ванъ деръ Верфъ, начало XVIII в.'), 500, 558, 245, 100, 90, 0.60),
  george: П(georgiy, т('«Чудо Георгія о змiѣ», Новгородъ, XV в.'), 250, 331, 125, 150, 200, 0.90),
  vladimir: П(lev, т('Гербъ Владиміра, 1781'), 500, 605, 250, 300, 590, 0.86, true),
  stanislav: П(mendeleev, т('Рѣпинъ, 1885'), 330, 395, 150, 115, 140, 0.70),
  first_income: П(lomonosov, т('Миропольскій, 1787'), 254, 409, 120, 130, 228, 0.74),
  plus_month: П(medved, т('Гербъ Ярославля, 1778'), 174, 214, 95, 107, 160, 0.80, true),
  plus_three: П(suvorov, т('Крейцингеръ, 1799'), 500, 652, 272, 212, 136, 0.72),
  diverse: П(tretyakov, т('Рѣпинъ, 1901'), 500, 416, 300, 125, 120, 0.62),
  big_deal: П(pushkin, т('Кипренскій, 1827'), 330, 384, 160, 125, 140, 0.70),
  goal_done: П(ermak, т('Парсуна, XVII в.'), 346, 480, 168, 120, 105, 0.60),
  tasks50: П(nahimov, т('Портретъ, XIX в.'), 500, 634, 250, 210, 379, 0.74),
  diverse5: П(ushakov, т('Бажановъ, 1912'), 500, 738, 235, 168, 145, 0.62),
  no_debt: П(olen, т('Гербъ Нижняго Новгорода'), 500, 655, 250, 330, 640, 0.86, true),
  year_million: П(oryol, т('Малый гербъ имперіи'), 250, 303, 125, 150, 291, 0.86, true),
  // --- ордена, прибавленные вторымъ заходомъ
  andrew: П(andrey, т('Ѳедоръ Зубовъ, 1669'), 500, 1050, 250, 152, 168, 0.62),
  catherine: П(ekaterina, т('Левицкій'), 500, 647, 255, 130, 170, 0.62),
  eagle: П(orelbel, т('Знакъ ордена Бѣлаго орла'), 500, 944, 235, 204, 253, 0.80),
  // --- отличія за дѣйствіе
  receipt10: П(speransky, т('Тропининъ'), 500, 639, 210, 122, 125, 0.62),
  split_first: П(kulibin, т('Веденецкій, около 1818'), 500, 630, 320, 165, 165, 0.66),
  streak30: П(dal, т('Перовъ, 1872'), 500, 577, 235, 200, 195, 0.68),
  inbox_zero: П(stolypin, т('Рѣпинъ, 1910'), 500, 766, 268, 215, 150, 0.62),
  // --- долговая вѣтка
  debt_closed: П(krylov, т('Брюлловъ, 1839'), 500, 604, 265, 222, 155, 0.64),
  debt_all_closed: П(sytin, т('Портретъ, начало XX в.'), 465, 550, 240, 155, 130, 0.62),
  clean_month: П(derzhavin, т('Боровиковскій, 1795'), 500, 611, 262, 145, 130, 0.62),
  pomodoro100: П(aivazovsky, т('Автопортретъ, 1874'), 500, 549, 235, 235, 200, 0.66),
  debt_falling: П(kankrin, т('Ботманъ, 1872'), 500, 693, 268, 205, 105, 0.58),
  // --- вѣхи мѣсячнаго заработка: русскіе промышленники и купцы
  month300: П(stroganov, т('мастерская Рослина'), 500, 610, 240, 180, 175, 0.64),
  month500: П(morozov, т('Фотографія, XIX в.'), 500, 527, 235, 165, 180, 0.66),
  month700: П(mamontov, т('Рѣпинъ, 1879'), 500, 638, 225, 175, 180, 0.66),
  month900: П(ryabushinsky, т('Дункерсъ, 1880'), 500, 655, 250, 155, 130, 0.62),
  month1000: П(kokorev, т('Тюринъ, 1889'), 500, 637, 268, 190, 130, 0.62),
  // --- накопленія
  save_first: П(vitte, т('Портретъ, начало XX в.'), 500, 719, 195, 150, 110, 0.60),
  save_cushion: П(kutuzov, т('Доу, 1829'), 500, 864, 260, 327, 70, 0.58),
  save_rate: П(bunge, т('Тюринъ, 1887'), 500, 610, 245, 190, 150, 0.64),
  save_growth: П(soldatenkov, т('Портретъ, XIX в.'), 500, 612, 245, 210, 190, 0.66),
  // --- орденъ Св. Александра Невскаго
  nevsky: П(nevsky, т('Боровиковскій'), 500, 652, 270, 120, 78, 0.55),
  // --- тайныя: знакъ открывается только вмѣстѣ съ наградой
  round_sum: П(demidov, т('Портретъ, XVIII в.'), 500, 631, 245, 165, 165, 0.64),
  archive_day: П(nestor, т('Гравюра Соколова'), 500, 765, 250, 305, 115, 0.60),
  comeback: П(dostoevsky, т('Перовъ, 1872'), 500, 623, 255, 215, 160, 0.66),
}
...
const ЛЕНТЫ: Record<string, { ribbon: string; name: string }> = {
  andrew: { ribbon: 'linear-gradient(90deg,#7fb3e0,#4b86c2 50%,#7fb3e0)', name: т('небесно-голубая') },
  george: { ribbon: полосы('#1c1a17', '#e08a2a', '#1c1a17', '#e08a2a', '#1c1a17'), name: т('чёрно-оранжевая') },
  vladimir: { ribbon: полосы('#1c1a17', '#b4322e', '#1c1a17'), name: т('чёрно-красная') },
  anna: { ribbon: полосы('#e0b44a', '#c8322b', '#c8322b', '#c8322b', '#e0b44a'), name: т('красная съ жёлтой каймой') },
  stanislav: { ribbon: полосы('#e8e2d6', '#c8322b', '#c8322b', '#c8322b', '#e8e2d6'), name: т('красная съ бѣлой каймой') },
  catherine: { ribbon: полосы('#cfd6dc', '#a8283a', '#a8283a', '#a8283a', '#cfd6dc'), name: т('красная съ серебряной каймой') },
  eagle: { ribbon: 'linear-gradient(90deg,#3a63a8,#22407a 50%,#3a63a8)', name: т('тёмно-синяя') },
  nevsky: { ribbon: 'linear-gradient(90deg,#d0433a,#a3241f 50%,#d0433a)', name: т('красная') },
}
...
export function Znak({ id, size, on, hidden, title }: ZnakProps) {
  ...
  const своя = [п?.from, лента && т('лента {0}', лента.name)].filter(Boolean).join(' · ')
  const подпись = hidden ? т('Тайное отличіе') : title ?? (своя || undefined)
  return (
    <span
      className={
