...
export async function модели(): Promise<Модель[]> {
  ...
  if (!r.ok) throw new Error(т('Ollama не отвечает'))
...
export async function спросить(
  ...
  if (!r.ok || !r.body) {
    throw new Error(т('Ollama ответила {0}. Модель «{1}» скачана?', r.status, модель))
  }
  ...
  if (пришло === 0) {
    throw new Error(
      причина === 'length'
        ? т('Модель упёрлась в предел, не начав отвечать. Возьмите модель поменьше или задайте вопрос короче.')
        : т('Модель вернула пустой ответ.'),
    )
  }
}
