...
import { anchor, bestSides, curveOf, inRect, lineOf, pointAt, type Curve } from './geometry'

/** Кривая связи с учётом формы (своей или доски). */
export function криваяСвязи(doc: CanvasDoc, e: CanvasEdge): Curve | null {
  const a = doc.nodes.find((n) => n.id === e.fromNode)
  const b = doc.nodes.find((n) => n.id === e.toNode)
  if (!a || !b) return null
  const shape = e.shape ?? doc.edgeShape ?? 'curve'
  return shape === 'line'
