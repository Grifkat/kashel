...
export function IconPicker({
  ...
  const подходитъ = (название: string, needle: string) =>
    название.toLowerCase().includes(needle) || т(название).toLowerCase().includes(needle)
  const groups = useMemo(() => {
    ...
    return ICON_GROUPS
      .map((g) => ({
        title: g.title,
        // Ищем и по-русски, и на языке окна: названия в каталоге русские,
        // а человек набирает то, что видит в подсказке.
        items: подходитъ(g.title, needle)
          ? g.items
          : g.items.filter((i) => подходитъ(i.title, needle)),
      }))
    ...
    <Modal title={т('Каталог иконок')} icon="palette" onClose={onClose}>
      ...
      {groups.map((g) => (
        <div key={g.title} style={{ marginBottom: 16 }}>
          <div className="card-title" style={{ marginBottom: 7 }}>{т(g.title)}</div>
          <div className="icon-grid">
            {g.items.map((i) => {
              const on = icon === i.id
              return (
