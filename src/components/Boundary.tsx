...
function CrashScreen({
  ...
  const save = async () => {
    ...
    try {
      await store.saveNow()
      setNote(т('Записано в хранилище.'))
    } catch (e) {
      setNote(т('Не удалось записать: ') + (e instanceof Error ? e.message : String(e)))
    }
  }
    ...
    return (
      <div className={slotClass}>
        ...
        <div className="empty" style={{ padding: 16, textAlign: 'left' }}>
          <div className="strong">{тр('{0} не собралась', where ?? т('Панель'))}</div>
          <div className="small faint" style={{ marginTop: 4 }}>
            {т('На остальную программу это не повлияло.')}</div>
          ...
          {canRetry ? (
            <button className="btn sm" style={{ marginTop: 10 }} onClick={onRetry}>
              {т('Попробовать снова')}</button>
          ) : (
            <div className="small faint" style={{ marginTop: 10 }}>
              {т('Причина не ушла — панель можно скрыть по Ctrl+I.')}</div>
          )}
        </div>
      </div>
    )
    ...
    return (
      <div className="view">
        <div className="advice-card alert">
          <div className="advice-title">{тр('Раздел «{0}» не отрисовался', where ?? т('без названия'))}</div>
          <div className="advice-body">
            {т('Остальная программа работает: вкладки, другие разделы и сохранение не затронуты. Записать данные можно кнопкой в строке состояния внизу или сочетанием Ctrl+S. Вкладку можно закрыть крестиком или переключиться на соседнюю — при возврате раздел откроется заново.')}</div>
          ...
          <div className="row" style={{ marginTop: 12 }}>
            {canRetry ? (
              <button className="btn sm" onClick={onRetry}>
                <Icon name="repeat" size={13} /> {т(' Попробовать снова')}</button>
            ) : (
              <span className="small faint">{т('Причина не ушла — раздел падает снова. Закройте вкладку.')}</span>
            )}
          </div>
        </div>
      </div>
    ...
    <div className="splash">
      <div className="card" style={{ maxWidth: 560, width: 'min(560px, 90vw)' }}>
        <div className="card-title">
          <Icon name="warn" size={14} /> {т(' Интерфейс не отрисовался')}</div>
        <div className="muted" style={{ lineHeight: 1.6 }}>
          {т('Хранилище не пострадало: данные в памяти целы, автосохранение продолжает работать. Запишите их на диск, прежде чем перезагружать окно.')}</div>
        ...
        <div className="row wrap" style={{ marginTop: 14, gap: 8 }}>
          ...
          <button className="btn primary" onClick={() => void save()}>
            <Icon name="save" size={14} /> {т(' Сохранить в хранилище')}</button>
          {canRetry && (
            <button className="btn" onClick={onRetry}>
              <Icon name="repeat" size={14} /> {т(' Продолжить работу')}</button>
          )}
          {isDesktop && (
        ...
        {!canRetry && (
          <div className="faint small" style={{ marginTop: 10, lineHeight: 1.55 }}>
            {т('Причина не ушла, падение повторяется — скорее всего дело в самих данных. Загляните в папку хранилища: чаще всего виновата запись, где не хватает поля, например операция без даты.')}</div>
        )}
      </div>
    </div>
  )
