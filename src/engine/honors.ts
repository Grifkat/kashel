...
export const RANKS: Record<RankBranch, string[]> = {
  civil: [
    т('Коллежскій регистраторъ'),
    т('Провинціальный секретарь'),
    т('Губернскій секретарь'),
    т('Корабельный секретарь'),
    т('Коллежскій секретарь'),
    т('Титулярный совѣтникъ'),
    т('Коллежскій асессоръ'),
    т('Надворный совѣтникъ'),
    т('Коллежскій совѣтникъ'),
    т('Статскій совѣтникъ'),
    т('Дѣйствительный статскій совѣтникъ'),
    т('Тайный совѣтникъ'),
    т('Дѣйствительный тайный совѣтникъ'),
    т('Канцлеръ'),
  ],
  military: [
    т('Прапорщикъ'),
    ...
    т('Бригадиръ'),
    т('Генералъ-маіоръ'),
    т('Генералъ-лейтенантъ'),
    т('Генералъ-аншефъ'),
    т('Генералъ отъ инфантеріи'),
    т('Генералъ-фельдмаршалъ'),
  ],
  merchant: [
    т('Коробейникъ'),
    т('Лавочникъ'),
    т('Мѣщанинъ'),
    т('Торговый гость'),
    т('Купецъ третьей гильдіи'),
    т('Купецъ второй гильдіи'),
    т('Купецъ первой гильдіи'),
    т('Почётный гражданинъ'),
    т('Потомственный почётный гражданинъ'),
    т('Поставщикъ Двора'),
    т('Коммерціи совѣтникъ'),
    т('Мануфактуръ-совѣтникъ'),
    т('Первогильдейскій промышленникъ'),
    т('Негоціантъ первой руки'),
  ],
}
...
export function address(level: number): string {
  ...
  if (cls >= 9) return т('Ваше благородіе')
  if (cls >= 6) return т('Ваше высокоблагородіе')
  if (cls === 5) return т('Ваше высокородіе')
  if (cls >= 3) return т('Ваше превосходительство')
  return т('Ваше высокопревосходительство')
}
...
export function xpBreakdown(data: VaultData, now: string = today()): XpPart[] {
  ...
  out.push({
    key: 'income',
    title: т('За заработанное'),
    xp: доход,
    hint: т('{0} мѣсяцевъ съ доходомъ', months.filter((m) => m.income > 0).length),
  })
  ...
  out.push({
    key: 'records',
    title: т('За ростъ и рекорды'),
    xp: ростъ * 150 + (лучшій > 0 ? 250 : 0),
    hint: ростъ ? т('{0} мѣсяцевъ лучше предыдущаго', ростъ) : т('ростъ пока не начался'),
  })
  ...
  out.push({
    key: 'sources',
    title: т('За независимость'),
    xp: Math.max(0, весомые - 1) * 200,
    hint: весомые === 1 ? т('{0} источникъ дохода', весомые) : т('{0} источниковъ дохода', весомые),
  })
  ...
  out.push({
    key: 'habit',
    title: т('За постоянство'),
    xp: дни * 5 + задачи * 15 + помидоры * 5 + цѣли * 300,
    hint: т('{0} дней съ записями, {1} закрытыхъ дѣлъ', дни, задачи),
  })
  ...
  out.push({
    key: 'quests',
    title: т('За заданія и серію'),
    xp: заданій * QUEST_XP + с.best * 10,
    hint: т('{0} закрытыхъ заданій, лучшая серія {1} дн.', заданій, с.best),
  })
  ...
  out.push({
    key: 'order',
    title: т('За порядокъ въ бумагахъ'),
    xp: чистыхъМѣсяцевъ * 100,
    hint: безъКатегоріи ? т('{0} записей безъ категоріи', безъКатегоріи) : т('всѣ записи разнесены'),
  })
  ...
  return [
    {
      ...
      hint: т('средній мѣсяцъ впередъ по прогнозу'),
    },
    {
      key: 'thrift',
      title: т('Бережливость'),
      value: шкала(Math.max(0, fc.planSavingsRate), 60),
      hint: т('норма сбереженій'),
    },
    {
      key: 'reserve',
      title: т('Запасъ прочности'),
      value: шкала(мѣсяцевъЗапаса, data.settings.profile.emergencyMonths || 6),
      hint: т('остатка хватитъ на {0} мѣс.', мѣсяцевъЗапаса.toFixed(1)),
    },
    {
      key: 'discipline',
      title: т('Дисциплина'),
      value: шкала(сЗаписями, дней * 0.5),
      hint: т('{0} дней съ записями за три мѣсяца', сЗаписями),
    },
    {
      key: 'order',
      title: т('Порядокъ въ бумагахъ'),
      value: Math.round(разнесено * 100),
      hint: безъКатегоріи ? т('{0} записей безъ категоріи', безъКатегоріи) : т('всё разнесено по статьямъ'),
    },
    {
      key: 'foresight',
      title: т('Прозорливость'),
      value: съЛимитомъ.length ? Math.round((въЛимитѣ / съЛимитомъ.length) * 100) : 0,
      hint: съЛимитомъ.length ? т('{0} изъ {1} статей въ предѣлѣ', въЛимитѣ, съЛимитомъ.length) : т('лимиты не назначены'),
    },
  ]
}
...
function ladder(
  ...
  return steps.map((s) => ({
    ...
    left: value >= s.at ? '' : т('осталось {0}', (s.unit ?? рубли)(s.at - value)),
  }))
}
...
export function awards(data: VaultData, now: string = today()): Award[] {
  ...
  const мѣс = (n: number) => т('{0} мѣс.', n.toFixed(1))
  const проц = (n: number) => `${n.toFixed(1)} %`
  const разы = (n: number) => `×${n.toFixed(1)}`
  const штукъ = (n: number) => `${Math.ceil(n)}`

  return [
    ...ladder('anna', т('Орденъ Св. Анны'), т('За доходъ, добытый трудомъ'), всегоДохода, [
      { at: 10_000_00, degree: 4, title: т('Анна 4-й степени') },
      { at: 100_000_00, degree: 3, title: т('Анна 3-й степени') },
      { at: 1_000_000_00, degree: 2, title: т('Анна 2-й степени') },
      { at: 5_000_000_00, degree: 1, title: т('Анна 1-й степени') },
    ]),
    /*
     ...
     * выдать её дважды.
     */
    ...ladder('george', т('Орденъ Св. Георгія'), т('За выдающійся мѣсяцъ'), лучшійМѣсяцъ, [
      { at: 50_000_00, degree: 4, title: т('Георгій 4-й степени') },
      { at: 120_000_00, degree: 3, title: т('Георгій 3-й степени') },
      { at: 250_000_00, degree: 2, title: т('Георгій 2-й степени') },
      { at: 500_000_00, degree: 1, title: т('Георгій 1-й степени') },
    ]),
    ...ladder('vladimir', т('Орденъ Св. Владиміра'), т('За запасъ прочности'), мѣсяцевъЗапаса, [
      { at: 1, degree: 4, title: т('Владиміръ 4-й степени'), unit: мѣс },
      { at: 3, degree: 3, title: т('Владиміръ 3-й степени'), unit: мѣс },
      { at: 6, degree: 2, title: т('Владиміръ 2-й степени'), unit: мѣс },
      { at: 24, degree: 1, title: т('Владиміръ 1-й степени'), unit: мѣс },
    ]),
    ...ladder('stanislav', т('Орденъ Св. Станислава'), т('За порядокъ въ бумагахъ'), дней, [
      { at: 30, degree: 3, title: т('Станиславъ 3-й степени'), unit: штукъ },
      { at: 150, degree: 2, title: т('Станиславъ 2-й степени'), unit: штукъ },
      { at: 400, degree: 1, title: т('Станиславъ 1-й степени'), unit: штукъ },
    ]),
    ...ladder('andrew', т('Орденъ Св. Андрея Первозваннаго'), т('За чистый капиталъ'), bal.net, [
      { at: 100_000_00, degree: 4, title: т('Андрей 4-й степени') },
      { at: 1_000_000_00, degree: 3, title: т('Андрей 3-й степени') },
      { at: 5_000_000_00, degree: 2, title: т('Андрей 2-й степени') },
      { at: 20_000_000_00, degree: 1, title: т('Андрей 1-й степени') },
    ]),
    /*
     ...
     */
    ...ladder('catherine', т('Орденъ Св. Екатерины'), т('За капиталъ, который самъ приноситъ'), доляКапитала, [
      { at: 5, degree: 4, title: т('Екатерина 4-й степени'), unit: проц },
      { at: 15, degree: 3, title: т('Екатерина 3-й степени'), unit: проц },
      { at: 30, degree: 2, title: т('Екатерина 2-й степени'), unit: проц },
      { at: 50, degree: 1, title: т('Екатерина 1-й степени'), unit: проц },
    ]),
    /*
     ...
     */
    ...ladder('nevsky', т('Орденъ Св. Александра Невскаго'), т('Изъ малыхъ денегъ — большія'), ростъОтъНачала, [
      { at: 3, degree: 4, title: т('Невскій 4-й степени'), unit: разы },
      { at: 5, degree: 3, title: т('Невскій 3-й степени'), unit: разы },
      { at: 10, degree: 2, title: т('Невскій 2-й степени'), unit: разы },
      { at: 20, degree: 1, title: т('Невскій 1-й степени'), unit: разы },
    ]),
    ...ladder('eagle', т('Орденъ Бѣлаго орла'), т('За ростъ дохода къ прошлому году'), ростъГода, [
      { at: 10, degree: 4, title: т('Бѣлый орелъ 4-й степени'), unit: проц },
      { at: 25, degree: 3, title: т('Бѣлый орелъ 3-й степени'), unit: проц },
      { at: 50, degree: 2, title: т('Бѣлый орелъ 2-й степени'), unit: проц },
      { at: 100, degree: 1, title: т('Бѣлый орелъ 1-й степени'), unit: проц },
    ]),
    // --- разовыя отличія
    {
      id: 'first_income',
      title: т('Первый доходъ'),
      about: т('Записанъ первый приходъ'),
      earned: всегоДохода > 0,
      progress: всегоДохода > 0 ? 1 : 0,
      left: всегоДохода > 0 ? '' : т('запишите первый приходъ'),
    },
    {
      id: 'plus_month',
      title: т('Мѣсяцъ сведёнъ въ плюсъ'),
      about: т('Заработано больше, чѣмъ истрачено'),
      earned: въПлюсѣ > 0,
      progress: въПлюсѣ > 0 ? 1 : 0,
      left: въПлюсѣ > 0 ? '' : т('ни одного мѣсяца въ плюсѣ'),
    },
    {
      id: 'plus_three',
      title: т('Три мѣсяца кряду въ плюсѣ'),
      about: т('Не случайность, а порядокъ'),
      earned: въПлюсѣ >= 3,
      progress: Math.min(1, въПлюсѣ / 3),
      left: въПлюсѣ >= 3 ? '' : т('осталось {0}', 3 - въПлюсѣ),
    },
    {
      id: 'diverse',
      title: т('Три источника дохода'),
      about: т('Одинъ заказчикъ — не доходъ, а зависимость'),
      earned: источники >= 3,
      progress: Math.min(1, источники / 3),
      left: источники >= 3 ? '' : т('осталось {0}', 3 - источники),
    },
    {
      id: 'big_deal',
      title: т('Гонораръ мечты'),
      about: т('Одинъ приходъ свыше ста тысячъ'),
      earned: крупнѣйшій >= 100_000_00,
      progress: Math.min(1, крупнѣйшій / 100_000_00),
      left: крупнѣйшій >= 100_000_00 ? '' : т('осталось {0}', рубли(100_000_00 - крупнѣйшій)),
    },
    {
      id: 'goal_done',
      title: т('Цѣль достигнута'),
      about: т('Задуманное доведено до конца'),
      earned: цѣлей > 0,
      progress: цѣлей > 0 ? 1 : 0,
      left: цѣлей > 0 ? '' : т('ни одной закрытой цѣли'),
    },
    {
      id: 'tasks50',
      title: т('Полсотни дѣлъ'),
      about: т('Пятьдесятъ закрытыхъ задачъ'),
      earned: дѣлъ >= 50,
      progress: Math.min(1, дѣлъ / 50),
      left: дѣлъ >= 50 ? '' : т('осталось {0}', 50 - дѣлъ),
    },
    {
      id: 'no_debt',
      title: т('Безъ долговъ'),
      about: т('Ни кредита, ни займа'),
      earned: долговъ === 0 && txs.length > 0,
      progress: долговъ === 0 ? 1 : 0,
      left: долговъ === 0 ? '' : т('остаётся закрыть {0}', долговъ),
    },
    /*
    ...
    ...([300, 500, 700, 900, 1000] as const).map((тыс) => ({
      id: `month${тыс}`,
      title: тыс === 500 ? т('Полмилліона за мѣсяцъ')
        : тыс === 1000 ? т('Милліонъ за мѣсяцъ')
        : т('{0} тысячъ за мѣсяцъ', { 300: т('Триста'), 700: т('Семьсотъ'), 900: т('Девятьсотъ') }[тыс]),
      about: т('Мѣсяцъ съ доходомъ отъ {0} ₽', (тыс * 1000).toLocaleString('ru-RU')),
      earned: лучшійМѣсяцъ >= тыс * 1000_00,
      progress: Math.min(1, лучшійМѣсяцъ / (тыс * 1000_00)),
      left: лучшійМѣсяцъ >= тыс * 1000_00 ? '' : т('осталось {0}', рубли(тыс * 1000_00 - лучшійМѣсяцъ)),
    })),
    {
      id: 'diverse5',
      title: т('Пять источниковъ дохода'),
      about: т('Пять статей, по которымъ шли приходы'),
      earned: источники >= 5,
      progress: Math.min(1, источники / 5),
      left: источники >= 5 ? '' : т('осталось {0}', 5 - источники),
    },
    // --- накопленія
    {
      id: 'save_first',
      title: т('Первый отложенный рубль'),
      about: т('На копилку переведены деньги'),
      earned: отложено,
      progress: отложено ? 1 : 0,
      left: копилки.length ? т('ни одного перевода на копилку') : т('нѣтъ ни одного счёта-копилки'),
    },
    {
      id: 'save_cushion',
      title: т('Подушка по вашей мѣркѣ'),
      about: т('Запаса хватитъ на {0} мѣс. — какъ и задумано въ профилѣ', подушка),
      earned: запасъМѣсяцевъ >= подушка,
      progress: подушка > 0 ? Math.min(1, запасъМѣсяцевъ / подушка) : 0,
      left: запасъМѣсяцевъ >= подушка ? '' : т('осталось {0} мѣс.', (подушка - запасъМѣсяцевъ).toFixed(1)),
    },
    {
      id: 'save_rate',
      title: т('Пятая часть отложена'),
      about: т('Норма сбереженій за годъ не ниже двадцати сотыхъ'),
      earned: норма >= 20,
      progress: Math.max(0, Math.min(1, норма / 20)),
      left: норма >= 20 ? '' : т('сейчасъ {0} %', норма.toFixed(1)),
    },
    {
      id: 'save_growth',
      title: т('Копилка растётъ'),
      about: т('Полгода подрядъ накопленій становилось больше'),
      earned: ростъНакопленій >= 6,
      progress: Math.min(1, ростъНакопленій / 6),
      left: ростъНакопленій >= 6 ? '' : т('подрядъ {0} изъ 6', ростъНакопленій),
    },
    {
      id: 'receipt10',
      title: т('Чекъ къ дѣлу'),
      about: т('Десять записей съ приложеннымъ чекомъ'),
      earned: съЧекомъ >= 10,
      progress: Math.min(1, съЧекомъ / 10),
      left: съЧекомъ >= 10 ? '' : т('осталось {0}', 10 - съЧекомъ),
    },
    {
      id: 'split_first',
      title: т('Раскладка'),
      about: т('Запись разложена по нѣсколькимъ статьямъ'),
      earned: съДолями > 0,
      progress: съДолями > 0 ? 1 : 0,
      left: съДолями > 0 ? '' : т('ни одной записи съ долями'),
    },
    {
      id: 'clean_month',
      title: т('Всё разнесено'),
      about: т('Мѣсяцъ безъ единой записи мимо статьи'),
      earned: чистыхъМѣсяцевъ > 0,
      progress: чистыхъМѣсяцевъ > 0 ? 1 : 0,
      left: чистыхъМѣсяцевъ > 0 ? '' : т('ни одного чистаго мѣсяца'),
    },
    {
      id: 'streak30',
      title: т('Ни дня безъ строчки'),
      about: т('Тридцать дней серіи подрядъ'),
      earned: серія.best >= 30,
      progress: Math.min(1, серія.best / 30),
      left: серія.best >= 30 ? '' : т('осталось {0}', 30 - серія.best),
    },
    {
      id: 'pomodoro100',
      title: т('Сто помидоровъ'),
      about: т('Сотня засчитанныхъ помидоровъ'),
      earned: помидоровъ >= 100,
      progress: Math.min(1, помидоровъ / 100),
      left: помидоровъ >= 100 ? '' : т('осталось {0}', 100 - помидоровъ),
    },
    {
      id: 'inbox_zero',
      title: т('Разгребено'),
      about: т('Во «Входящихъ» не осталось ни одного дѣла'),
      earned: задачъ > 0 && входящихъ === 0,
      progress: задачъ > 0 && входящихъ === 0 ? 1 : 0,
      left: задачъ === 0 ? т('дѣлъ пока нѣтъ') : т('осталось разобрать {0}', входящихъ),
    },
    {
      id: 'debt_closed',
      title: т('Долгъ платежомъ красенъ'),
      about: т('Первый долговой счётъ сведёнъ въ ноль'),
      earned: закрытыхъДолговъ > 0,
      progress: закрытыхъДолговъ > 0 ? 1 : 0,
      left: долговые.length ? т('ни одинъ долгъ ещё не сведёнъ') : т('долговыхъ счетовъ нѣтъ'),
    },
    {
      id: 'debt_all_closed',
      title: т('Послѣдній рубль долга'),
      about: т('Всѣ долговые счета сведены въ ноль'),
      earned: всеСведены,
      ...
      left: всеСведены
        ? ''
        : долговые.length
          ? т('остаётся свести {0}', долговые.length - закрытыхъДолговъ)
          : т('долговыхъ счетовъ нѣтъ'),
    },
    {
      id: 'debt_falling',
      title: т('Долгъ убываетъ'),
      about: т('Долговъ меньше, чѣмъ полгода назадъ'),
      earned: долгъУбылъ,
      progress: долгъБылъ > 0 ? Math.min(1, Math.max(0, (долгъБылъ - долгъТеперь) / долгъБылъ)) : 0,
      left: долгъБылъ > 0 ? т('сейчасъ {0}, было {1}', рубли(долгъТеперь), рубли(долгъБылъ)) : т('полгода назадъ долговъ не было'),
    },
    // --- тайныя: названіе и условіе скрыты, пока не сойдётся
    {
      id: 'round_sum',
      secret: true,
      title: т('Ровно сто тысячъ'),
      about: т('Приходъ ровно въ сто тысячъ рублей'),
      earned: круглый,
    ...
    },
    {
      id: 'archive_day',
      secret: true,
      title: т('Годъ въ одинъ день'),
      about: т('За одинъ заходъ внесены записи болѣе чѣмъ за шестьдесятъ датъ'),
    ...
    },
    {
      ...
      secret: true,
      title: т('Возвращеніе'),
      about: т('Послѣ мѣсяца молчанія — недѣля записей'),
      earned: вернулся,
    ...
    {
      id: 'year_million',
      title: т('Милліонъ за годъ'),
      about: т('Годовой доходъ свыше милліона'),
      earned: годъ.income >= 1_000_000_00,
      progress: Math.min(1, годъ.income / 1_000_000_00),
      left: годъ.income >= 1_000_000_00 ? '' : т('осталось {0}', рубли(1_000_000_00 - годъ.income)),
    },
  ]
}
...
export function titles(data: VaultData, now: string = today()): { key: string; title: string; earned: boolean }[] {
  ...
  return [
    { key: 'income', title: т('Промышленникъ'), earned: по('income') >= 80 },
    { key: 'thrift', title: т('Рачительный хозяинъ'), earned: по('thrift') >= 80 },
    { key: 'reserve', title: т('Хранитель казны'), earned: по('reserve') >= 80 },
    { key: 'discipline', title: т('Неусыпный'), earned: по('discipline') >= 80 },
    { key: 'order', title: т('Столоначальникъ'), earned: по('order') >= 80 },
    { key: 'foresight', title: т('Прозорливый'), earned: по('foresight') >= 80 },
  ]
}
...
const НАБОРЪ: Заданіе[] = [
  {
    id: 'entries20',
    title: т('Двадцать записей'),
    about: т('Внести за мѣсяцъ двадцать операцій'),
    progress: (d, мк) => Math.min(1, мѣсяцаЗаписи(d, мк).length / 20),
  },
  {
    id: 'month_plus',
    title: т('Мѣсяцъ въ плюсъ'),
    about: т('Заработать больше, чѣмъ истратить'),
    progress: (d, мк) => {
  ...
  {
    id: 'clean_month',
    title: т('Ничего мимо статьи'),
    about: т('Разнести всѣ записи мѣсяца по статьямъ'),
    progress: (d, мк) => {
  ...
  },
  {
    id: 'tasks10',
    title: т('Десять дѣлъ'),
    about: т('Закрыть за мѣсяцъ десять задачъ'),
    progress: (d, мк) => {
  ...
  {
    id: 'in_limits',
    title: т('Не выйти за смѣту'),
    about: т('Уложиться во всѣ назначенные лимиты'),
    progress: (d, мк) => {
  ...
  {
    id: 'save10',
    title: т('Отложить десятую часть'),
    about: т('Оставить отъ дохода мѣсяца хотя бы десять сотыхъ'),
    progress: (d, мк) => {
      const свои = мѣсяцаЗаписи(d, мк)
      const приходъ = свои.filter((t) => t.kind === 'income').reduce((s, t) => s + t.amount, 0)
  ...
  {
    id: 'five_days',
    title: т('Пять дней кряду'),
    about: т('Пять дней подрядъ съ записями внутри мѣсяца'),
    progress: (d, мк) => {
  ...
  {
    id: 'new_source',
    title: т('Новый источникъ'),
    about: т('Приходъ по статьѣ, которой раньше не было'),
    progress: (d, мк) => {
...
export function quests(data: VaultData, now: string = today()): Quest[] {
  ...
  for (const t of (data.tasks ?? []).filter((x) => !x.done).slice(0, 5)) {
    out.push({
      id: 'task_' + t.id,
      title: t.title,
      about: t.amount ? (t.moneyKind === 'income' ? т('приходъ {0} ₽', рубли(t.amount)) : т('трата {0} ₽', рубли(t.amount))) : т('дѣло безъ суммы'),
