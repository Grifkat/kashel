...
import { НАСТАВЛЕНІЕ, сводкаДляМодели } from '../src/engine/svodka'
...
// --- чего быть не должно ни при какихъ условіяхъ
const не = (что: string, игла: string) =>
  check(что, !сводка.текстъ.includes(игла), игла)

не('подложенный комментарій не уходитъ', тайныйКомментарій)
не('названіе задачи не уходитъ', тайнаяЗадача)
не('заметки не уходятъ', тайнаяЗамѣтка)

/*
 * И то же самое сплошнякомъ, а не по одной подложенной строкѣ.
 *
 * Первый заходъ провѣрялъ только свой тайный комментарій — и прошёлъ мимо
 * утечки, при которой въ сводку легли первыя полсотни покупокъ: подложенная
 * была послѣдней и въ выборку не попала. Провѣрка, которую можно обойти
 * порядкомъ строкъ, ничего не стережётъ. Здѣсь берутся всѣ комментаріи разомъ.
 */
/*
 * Изъ провѣрки убираются комментаріи, совпадающіе съ названіями статей,
 * счетовъ и цѣлей. Такія слова стоятъ въ сводкѣ по праву — «Зарплата» тамъ
 * заголовокъ статьи, а не чей-то комментарій; считать ихъ утечкой значило бы
 * получить ложную тревогу и потомъ отучиться ей вѣрить.
 */
const законныя = new Set([
  ...дляСводки.categories.map((c) => c.name),
  ...дляСводки.accounts.map((a) => a.name),
  ...дляСводки.goals.map((g) => g.name),
  ...дляСводки.recurring.map((r) => r.title),
])
const всѣКомментаріи = дляСводки.transactions
  .map((t) => (t.note ?? '').trim())
  .filter((н) => н.length > 3 && !законныя.has(н))
const утекло = всѣКомментаріи.filter((н) => сводка.текстъ.includes(н))
check('ни одинъ комментарій къ покупкѣ не уходитъ',
  всѣКомментаріи.length > 0 && утекло.length === 0,
  `проверено ${всѣКомментаріи.length}, утекло ${утекло.length}${утекло.length ? ': ' + утекло[0] : ''}`)

const всѣЗадачи = дляСводки.tasks.map((t) => t.title).filter((т) => т.length > 3)
check('ни одно названіе задачи не уходитъ',
  всѣЗадачи.every((т) => !сводка.текстъ.includes(т)), `проверено ${всѣЗадачи.length}`)

// Отдѣльныхъ операцій быть не должно: суммы по статьямъ — да, покупки — нѣтъ.
const мелкихъДатъ = (сводка.текстъ.match(/\d{4}-\d{2}-\d{2}/g) ?? [])
check('въ сводкѣ нѣтъ россыпи датъ отдѣльныхъ покупокъ',
  мелкихъДатъ.length <= 4, `дат: ${мелкихъДатъ.length}`)
