...
import { creditState, schedule, whatIf, следующийПлатёж, сводкаКредита, перевестиКредиты, планъПлатежа, разложитьПлатёжъ, датыПлатежей, остатокПослѣ, подсказкаОстатка, этоПлатёжПо, СТАТЬЯ_ПЛАТЕЖЕЙ, СТАТЬЯ_ПРОЦЕНТОВ } from '../src/engine/credit'
...
function разборОшибокъ() {
  ...
  // «Мне должны» — заморожено: в чистом капитале есть, в деньгах на руках нет.
  check('«мне должны» — заморожено, «я должен» — обязательство',
    итоги.assets === 0 && итоги.lent === 3_000_00 && итоги.liabilities === -5_000_00 && итоги.net === -2_000_00)
...
function погашениеПроверка() {
  console.log('\n— погашение без счёта, просрочка, долги —')
  const карта = { ...data.accounts.find((a) => a.type === 'card')!, id: 'card1', initialBalance: 100_000_00, archived: false, project: undefined }
  const кредит = (purpose: 'purchase' | 'cash', over: Record<string, unknown> = {}) => ({
    id: 'cr_' + purpose, name: 'Кредит', type: 'credit' as const, icon: 'x', color: '#000', initialBalance: -60_000_00,
    credit: { kind: 'loan' as const, principal: 60_000_00, ratePct: 0, termMonths: 12, startDate: '2025-12-20', paymentDay: 10,
      monthlyPayment: 5_000_00, purpose, v: 2 as const, trackFrom: '2026-01-01', ...over },
  })
  const покупка = кредит('purchase')
  const деньгами = кредит('cash', { ratePct: 12 })
  const база = { ...data, accounts: [карта, покупка, деньгами], transactions: [] as Transaction[], categories: data.categories, notifications: [] }
  const записать = (d: typeof база, ops: Omit<Transaction, 'id' | 'createdAt'>[]) =>
    ({ ...d, transactions: [...d.transactions, ...ops.map((o, i) => ({ ...o, id: 'x' + d.transactions.length + '_' + i, createdAt: '2026-01-01' }))] })

  // покупка, без счёта
  const п1 = планъПлатежа({ кредитъ: покупка, счётъ: null, сумма: 5_000_00, дата: '2026-01-10', data: база })!
  check('платёж без счёта: одна операция на самом кредите с пометкой', п1.операціи.length === 1 && п1.операціи[0].accountId === покупка.id && п1.операціи[0].offBook === 'out')
  const д1 = записать(база, п1.операціи)
  check('платёж без счёта гасит долг', creditRemaining(покупка, д1.transactions) === 55_000_00)
  check('и не трогает ваши счета', accountBalance(карта, д1.transactions) === 100_000_00)
  check('платёж по покупке без счёта — всё равно расход', categoryTotals(д1.transactions, 'expense').reduce((s, x) => s + x.amount, 0) === 5_000_00)
  check('платёж без счёта считается платежом', этоПлатёжПо(покупка, д1.transactions[0]))

  // деньгами, без счёта, с процентами и штрафом
  const п2 = планъПлатежа({ кредитъ: деньгами, счётъ: null, сумма: 5_000_00, дата: '2026-01-10', data: база, штраф: 700_00, статьяШтрафа: СТАТЬЯ_ШТРАФОВ })!
  const д2 = записать(база, п2.операціи)
  const проценты = Math.round(60_000_00 * 0.01)
  check('кредит деньгами без счёта: тело гасит долг', creditRemaining(деньгами, д2.transactions) === 60_000_00 - (5_000_00 - проценты))
  check('в расходы — проценты и штраф, но не тело', п2.расходъ === проценты + 700_00 &&
    categoryTotals(д2.transactions, 'expense').reduce((s, x) => s + x.amount, 0) === проценты + 700_00)
  check('штраф — своя статья и долг не гасит', п2.статьи.some((c) => c.id === СТАТЬЯ_ШТРАФОВ.id) && п2.операціи.some((о) => о.categoryId === СТАТЬЯ_ШТРАФОВ.id && о.debtPrincipal === 0))
  check('карта не тронута', accountBalance(карта, д2.transactions) === 100_000_00)
  const д2к = записать(база, планъПлатежа({ кредитъ: деньгами, счётъ: 'card1', сумма: 5_000_00, дата: '2026-01-10', data: база, штраф: 700_00, статьяШтрафа: СТАТЬЯ_ШТРАФОВ })!.операціи)
  check('со счёта — с карты уходит платёж и штраф', accountBalance(карта, д2к.transactions) === 100_000_00 - 5_700_00)

  // просрочка
  const нет = просрочкаКредита(покупка, база, '2026-03-15')!
  check('три прошедших платежа без оплаты — просрочка 15 000', нет.сумма === 15_000_00 && нет.платежей === 3 && нет.с === '2026-01-10')
  const одинВнесён = записать(база, п1.операціи)
  const пр = просрочкаКредита(покупка, одинВнесён, '2026-03-15')!
  check('внесён один — просрочены два, с февраля', пр.сумма === 10_000_00 && пр.платежей === 2 && пр.с === '2026-02-10')
  check('окно предлагает просроченное и ближайший', пр.предложить === 15_000_00)
  check('сегодняшний платёж ещё не просрочен', просрочкаКредита(покупка, одинВнесён, '2026-02-10')!.сумма === 0)
  const недоплата = записать(база, [{ ...п1.операціи[0], amount: 3_000_00, debtPrincipal: 3_000_00 }])
  check('недоплата — тоже просрочка', просрочкаКредита(покупка, недоплата, '2026-01-20')!.сумма === 2_000_00)
  check('до отметки учёта просрочки нет', просрочкаКредита({ ...покупка, credit: { ...покупка.credit, trackFrom: '2026-03-01' } }, база, '2026-03-15')!.сумма === 5_000_00)
  check('без отметки учёта — не выдумываем', просрочкаКредита({ ...покупка, credit: { ...покупка.credit, trackFrom: undefined } }, база, '2026-03-15')!.сумма === 0)
  check('у кредитной карты просрочки по графику нет', просрочкаКредита({ ...покупка, credit: { ...покупка.credit, kind: 'card' } }, база, '2026-03-15') === null)
  const сПроцентами = записать(база, планъПлатежа({ кредитъ: деньгами, счётъ: 'card1', сумма: 5_000_00, дата: '2026-01-10', data: база })!.операціи)
  check('проценты входят во внесённое — полный платёж не просрочен', просрочкаКредита(деньгами, сПроцентами, '2026-01-20')!.сумма === 0)
  check('штраф во внесённое не входит', просрочкаКредита(деньгами, записать(база, п2.операціи.filter((о) => о.categoryId === СТАТЬЯ_ШТРАФОВ.id)), '2026-01-20')!.сумма === 5_000_00)
  const отмечены = отметитьУчётКредитов([{ ...покупка, credit: { ...покупка.credit, trackFrom: undefined } }, карта], '2026-09-17')
  check('отметка учёта ставится один раз и только кредитам', отмечены.changed === 1 && отмечены.accounts[0].credit?.trackFrom === '2026-09-17')

  // долги людям
  const мне = новыйДолг({ id: 'd_me', кто: ' Дима ', направление: 'owed_to_me' })
  const я = новыйДолг({ id: 'd_i', кто: 'Петя', направление: 'i_owe', срок: '2026-12-01' })
  check('новый долг — без суммы, сверен, имя без пробелов', мне.initialBalance === 0 && мне.debt?.reviewed === true && мне.name === 'Дима' && я.debt?.dueDate === '2026-12-01')
  const сДолгами = { ...база, accounts: [карта, мне, я] }
  const шаг = (d: typeof сДолгами, долг: typeof мне, действие: 'repay' | 'borrow' | 'collect' | 'lend', счёт: string | null, сумма: number) =>
    записать(d, [планДолга({ долг, действие, сумма, дата: '2026-02-01', счёт })!])
  let дд = шаг(сДолгами, мне, 'lend', 'card1', 3_000_00)
  check('дали в долг со счёта: карта −3 000, долг вам 3 000', accountBalance(карта, дд.transactions) === 97_000_00 && остатокДолга(мне, дд.transactions) === 3_000_00)
  let бал = balances(дд.accounts, дд.transactions)
  check('отданное в долг не входит в «Итого», а стоит отдельно', бал.assets === 97_000_00 && бал.lent === 3_000_00 && бал.net === 100_000_00)
  дд = шаг(дд, мне, 'lend', null, 2_000_00)
  check('дали ещё без счёта: долг растёт, карта нет', остатокДолга(мне, дд.transactions) === 5_000_00 && accountBalance(карта, дд.transactions) === 97_000_00)
  дд = шаг(дд, мне, 'collect', 'card1', 1_000_00)
  check('получили на счёт: долг меньше, карта больше', остатокДолга(мне, дд.transactions) === 4_000_00 && accountBalance(карта, дд.transactions) === 98_000_00)
  дд = шаг(дд, мне, 'collect', null, 4_000_00)
  check('получили мимо счетов: долг закрыт, карта та же', остатокДолга(мне, дд.transactions) === 0 && accountBalance(карта, дд.transactions) === 98_000_00)
  дд = шаг(дд, я, 'borrow', 'card1', 10_000_00)
  check('взяли в долг на счёт: карта +10 000, вы должны 10 000', accountBalance(карта, дд.transactions) === 108_000_00 && остатокДолга(я, дд.transactions) === 10_000_00)
  дд = шаг(дд, я, 'repay', null, 4_000_00)
  check('отдали мимо счетов: долг 6 000, карта та же', остатокДолга(я, дд.transactions) === 6_000_00 && accountBalance(карта, дд.transactions) === 108_000_00)
  check('долг — не расход и не доход', categoryTotals(дд.transactions, 'expense').length === 0 && categoryTotals(дд.transactions, 'income').length === 0)
  check('чужое действие для долга не записывается', планДолга({ долг: мне, действие: 'repay', сумма: 1, дата: '2026-01-01', счёт: null }) === null)
  check('счёт-долг сам себе не источник', планДолга({ долг: мне, действие: 'lend', сумма: 1, дата: '2026-01-01', счёт: 'd_me' }) === null)

  // сверка старых долгов
  const старый = { ...мне, id: 'd_old', initialBalance: 3_000_00, debt: { ...мне.debt!, reviewed: undefined } }
  const сверка = { ...база, accounts: [карта, старый, { ...старый, id: 'd_zero', initialBalance: 0 }] }
  check('сверить просят только долги прежних версий с суммой', несверенныеДолги(сверка).map((a) => a.id).join() === 'd_old')
  const безСчёта = сверитьДолг(сверка, 'd_old', null)
  check('«счета не менять» — только отметка', безСчёта.операция === null && безСчёта.accounts.find((a) => a.id === 'd_old')?.debt?.reviewed === true)
  const сосчёта = сверитьДолг(сверка, 'd_old', 'card1', '2026-02-01')
  const послеСверки = записать({ ...сверка, accounts: сосчёта.accounts }, [сосчёта.операция!])
  check('сверка со счётом списывает сумму с карты, долг прежний',
    accountBalance(карта, послеСверки.transactions) === 97_000_00 && остатокДолга(старый, послеСверки.transactions) === 3_000_00 && несверенныеДолги(послеСверки).length === 0)

  // архив хранит новое
  const архив = { kashel: 'vault', formatVersion: 1, app: 'Кошель', exportedAt: '2026-09-14T00:00:00Z', counts: {},
    data: { ...data, accounts: [покупка, мне], transactions: д1.transactions }, notes: {}, canvases: {}, attachments: {} }
  const р = parseArchive(JSON.stringify(архив))
  const ра = р.ok ? р.archive.data : null
  check('архив хранит пометку «без счёта», отметку учёта и сверку',
...
function ставкаПоПлатежу() {
  ...
  // Обратный ход: берём ставку, считаем платёж — и подбором получаем её же.
  for (const годовых of [5, 12, 19.9, 35]) {
    const платёж = Math.round(платёжАннуитета(300_000_00, годовых / 12 / 100, 24))
    const назад = подобратьСтавку(300_000_00, платёж, 24)
    check(`ставка {${годовых}%} восстанавливается по платежу`, назад != null && Math.abs(назад - годовых) < 0.05, String(назад))
  }
  const яндекс = подобратьСтавку(196_093_61, 12_084_00, 17)
  check('рассрочка «Яндекса»: 17 платежей по 12 084 ₽ на 196 093,61 ₽ — около 6,2%',
    яндекс != null && Math.abs(яндекс - 6.2) < 0.3, String(яндекс))
  check('платежей ровно на долг — ставки нет', подобратьСтавку(120_000_00, 10_000_00, 12) === null)
  check('платежей меньше долга — ставки нет', подобратьСтавку(120_000_00, 9_000_00, 12) === null)
  check('пустые числа — ставки нет', подобратьСтавку(0, 1000, 12) === null && подобратьСтавку(1000, 0, 12) === null && подобратьСтавку(1000, 100, 0) === null)

  const кредит = {
    id: 'cr_st', name: 'Яндекс 200', type: 'credit' as const, icon: 'x', color: '#000', initialBalance: -196_093_61,
    credit: { kind: 'loan' as const, principal: 200_000_00, ratePct: 0, termMonths: 17, startDate: '2026-09-05', paymentDay: 5,
      monthlyPayment: 12_084_00, purpose: 'purchase' as const, v: 2 as const, trackFrom: '2026-09-05' },
  }
  const безСуммы = { ...кредит, credit: { ...кредит.credit, principal: 0, termMonths: 0 } }
  const п = прикидкаСтавки(безСуммы, { ...data, accounts: [безСуммы], transactions: [] }, '2026-09-18')
  check('без суммы договора прикидка считает по остатку', п?.по === 'left' && п?.платежей === 17 && п?.долг === 196_093_61
    && !!п && Math.abs(п.ставка - 6.2) < 0.3, JSON.stringify(п))
  check('и считает переплату', п?.переплата === 12_084_00 * 17 - 196_093_61)
  const поДоговору = прикидкаСтавки(кредит, { ...data, accounts: [кредит], transactions: [] }, '2026-09-18')
  check('с суммой и сроком считает по договору', поДоговору?.по === 'contract' && поДоговору?.долг === 200_000_00 && поДоговору?.платежей === 17)
  const беспроцентный = прикидкаСтавки(
    { ...кредит, initialBalance: -120_000_00, credit: { ...кредит.credit, monthlyPayment: 10_000_00, termMonths: 12 } },
    { ...data, accounts: [], transactions: [] }, '2026-09-18')
  check('у настоящей рассрочки без процентов подсказки нет', беспроцентный === null)
}
