  ...
  const template = [
    ...
    },
    {
      ...
      submenu: [
         ...
         *
         * Сама проверка тут не делается: пункт лишь просит окно открыть
         * «Настройки» и начать. Иначе рядом с уже готовым разделом появился
...
ipcMain.handle('update:install', (_e, н) => поЯзыку(async () => {
  ...
  // Мак: открыть скачанный образ — Finder покажет окно с программой и
  // «Программами», куда её перетащить.
  if (итогъ.открыть) void shell.openPath(итогъ.путь)
